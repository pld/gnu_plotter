...
private
    def assign_options(plot, options)
      ...
      # [:title, :xlabel, :ylabel, :logscale, :log].each do |k|
      #   v = options.delete(k)
      #   v && plot.send(k, v.remove_single_quotes)
      # end
      # [:yrange, :xrange].each do |k|
      #   v = options.delete(k)
      #   v && plot.send(k, "[#{v.join(':')}]")
      # end
      # plot.xtics tics(options[:xtics]) if options[:xtics]
      # plot.ytics tics_for_data(data) if options[:ytics]
      [plot, data]
    end
